select p_date, sum(case when col_flag = 1 then col_num else 0 end), sum(case when col_flag = 2 then col_num else 0 end), sum(case when col_flag = 3 then col_num else 0 end)
from (
	select replace(date_add(p_date, interval 15 day),'-','') as p_date, col_flag, count(*) as col_num
		from
		(
			...
			( 
					select concat(substring(p_date, 1, 4), '-', substring(p_date, 5, 2), '-', substring(p_date, 7, 2)) as p_date, col_uid, 1 as col_flag 
						...
						where col_new_user = 1 and p_date >= {date-21} and p_date <= {date-15}
	...
	group by replace(date_add(p_date, interval 15 day),'-',''), col_flag )tc
group by p_date