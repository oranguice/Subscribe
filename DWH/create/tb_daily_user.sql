select e.p_date, e.col_uid, e.col_udid, e.col_version, e.col_feed_card_num, e.col_content_card_num, e.col_feed_clicks, e.col_feed_impressions, e.col_startpage_distribution, e.col_startpage_detail_distribution, e.col_publisher_distribution, e.col_publisher_detail_distribution, e.col_subset_distribution, e.col_subset_detail_distribution, f.col_launches, e.col_new_user
from (
	select c.p_date , c.col_uid, c.col_udid, c.col_version, c.col_feed_card_num, c.col_content_card_num, case when c.col_udid is not null and d.col_feed_clicks is null then 0L else d.col_feed_clicks end, c.col_feed_impressions, case when c.col_udid is not null and d.col_startpage_distribution is null then 0L else d.col_startpage_distribution end, case when c.col_udid is not null and d.col_startpage_detail_distribution is null then 0L else d.col_startpage_detail_distribution end, case when c.col_udid is not null and d.col_publisher_distribution is null then 0L else d.col_publisher_distribution end, case when c.col_udid is not null and d.col_publisher_detail_distribution is null then 0L else d.col_publisher_detail_distribution end, case when c.col_udid is not null and  d.col_subset_distribution is null then 0L else d.col_subset_distribution end, case when c.col_udid is not null and d.col_subset_detail_distribution is null then 0L else d.col_subset_detail_distribution end, c.col_new_user
	from (
	...
	left outer join (
		...
		sum(case when col_name in ('安装','播放') and col_startpage_card_feed_name = 'SubscribeFeed' then 1 else 0 end) as col_startpage_distribution,
		sum(case when col_name in ('安装','播放') and col_url_normalize like '/detail%' and col_from_url_normalize in ('/explore/subscribe_page','/explore/startpage') then 1 else 0 end) as col_startpage_detail_distribution,
