select g.p_date, g.col_uid, g.col_udid, g.col_version, g.col_feed_card_num, g.col_content_card_num, g.col_feed_clicks, g.col_feed_impressions, g.col_subscribe_page_distribution, g.col_subscribe_page_detail_distribution, g.col_publisher_distribution, g.col_publisher_detail_distribution, g.col_subset_distribution, g.col_subset_detail_distribution, g.col_launches, g.col_new_user, h.is_cheating_latest
from (
	select e.p_date, e.col_uid, e.col_udid, e.col_version, e.col_feed_card_num, e.col_content_card_num, case when e.col_udid is not null and e.col_feed_clicks is null then 0L else e.col_feed_clicks end as col_feed_clicks, e.col_feed_impressions, case when e.col_udid is not null and e.col_subscribe_page_distribution is null then 0L else e.col_subscribe_page_distribution end as col_subscribe_page_distribution, case when e.col_udid is not null and e.col_subscribe_page_detail_distribution is null then 0L else e.col_subscribe_page_detail_distribution end as col_subscribe_page_detail_distribution, case when e.col_udid is not null and e.col_publisher_distribution is null then 0L else e.col_publisher_distribution end as col_publisher_distribution, case when e.col_udid is not null and e.col_publisher_detail_distribution is null then 0L else e.col_publisher_detail_distribution end as col_publisher_detail_distribution, case when e.col_udid is not null and e.col_subset_distribution is null then 0L else e.col_subset_distribution end as col_subset_distribution, case when e.col_udid is not null and e.col_subset_detail_distribution is null then 0L else e.col_subset_detail_distribution end as col_subset_detail_distribution, f.col_launches, e.col_new_user
	from (
	...
	on e.p_date = f.p_date and e.col_uid = f.col_uid and e.col_udid = f.col_udid) g
left outer join (
	select p_date, col_udid, is_cheating_latest
	from android.android_application_start_cheating
	where p_date = {date}
	group by p_date, col_udid, is_cheating_latest) h
on g.p_date = h.p_date and g.col_udid = h.col_udid