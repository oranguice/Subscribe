...
from (
    ...
    where p_date >= 20150201 and p_date <= 20150204 and ((col_url like '/topic?id=%' and length(col_url) > 15) or col_url like '/subset_profil%')) a
join (
    select p_date, col_uid
    from subscribe.tb_daily_users
    where p_date >= 20150201 and p_date <= 20150204 and col_new_user = 0) b
on a.p_date = b.p_date and a.col_uid = b.col_uid
group by a.p_date
--订阅用户中看到：新专题人数；次数；老专题人数；次数
...
on a.p_date = b.p_date and a.col_uid = b.col_uid
group by a.p_date
--订阅用户中点击：新专题关注、内容、消费、下一个、更多；老专题关注、内容、消费
...
where b.col_udid is null
group by a.p_date
--非订阅用户中点击：新专题关注、内容、消费、下一个、更多；老专题关注、内容、消费
