 ...
 from (
    ...
    from (
            ...
            where p_date = {date} and col_name = '安装' and col_startpage_card_id like '%ACCOUNT:{uid}%'
            group by col_udid
        union all
...
union all
    ...
    from (
            select col_udid, count(*) as col_downloads
            from android.tb_click
            where p_date = {date} and col_name = '安装' and col_url = '/publisher_profile?id={uid}'
            group by col_udid
        union all
            select col_udid, count(*) as col_downloads
            from android.tb_click
            where p_date = {date} and col_name = '安装' and col_url like '/detail/app?pn=%' and col_from_url = '/publisher_profile?id={uid}'
            group by col_udid) d
    group by col_udid
union all
    ...
    from (
            ...
            from android.tb_click
            where p_date = {date} and (col_url_normalize = '/topic?id=' or col_url_normalize = '/subset_profile?id=') and col_name = '安装'
        union all
            ...
            from android.tb_click
            where p_date = {date} and col_name = '安装' and col_url like '/detail/app?pn=%' and (col_from_url_normalize = '/topic?id=' or col_from_url_normalize = '/subset_profile?id=')
        ) e join (
            select distinct col_card_id
            from (
                    select substring(col_startpage_card_id, 33, 18) as col_card_id
                    from android.tb_card_show
                    where col_startpage_card_id like 'SubscribeFeed_v1-SUBSCRIBE_FEED%ACCOUNT:{uid}%' and p_date = {date}
                union all
                    select substring(col_startpage_card_id, 52, 18) as col_card_id
                    from android.tb_card_show
                    where col_startpage_card_id like 'SubscribeFeed_v1-SUBSCRIBE_FEED%ACCOUNT:{uid}%' and length(col_startpage_card_id) - length(regexp_replace(col_startpage_card_id, '#', '')) >= 1 and p_date = {date}
                union all
